    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">
              Probability-driven insights into potential crime linkages across active investigations.
            </p>
          </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        {linkedItems.map(item => (
          <div key={`${item.sourceCaseId}-${item.targetCaseId}`} className="rounded-3xl border border-slate-800/80 bg-slate-900/70 p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <div className="text-sm uppercase tracking-[0.24em] text-slate-500">Link confidence</div>
                <h2 className="mt-2 text-xl font-semibold text-white">{item.sourceTitle} ↔ {item.targetTitle}</h2>
              </div>
              <AlertBadge
            ...
            <div className="mt-4 grid gap-3 text-sm text-slate-300">
              ...
              <div className="rounded-2xl border border-slate-800/80 bg-slate-950/70 p-3 text-slate-400">
                This analysis reflects probability only and should not be treated as a confirmed match.
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="rounded-3xl border border-slate-800/80 bg-slate-900/70 p-6">
        <h2 className="text-xl font-semibold text-white">Related case summary</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {cases.map(caseItem => (
            <div key={caseItem.id} className="rounded-2xl border border-slate-800/80 bg-slate-950/70 p-4">
              <div className="text-sm text-slate-400">{caseItem.id}</div>
              <div className="mt-2 font-semibold text-white">{caseItem.title}</div>
              <div className="mt-1 text-sm text-slate-400">{caseItem.status} • {caseItem.location}</div>
            </div>
          ))}
        </div>
      </div>
